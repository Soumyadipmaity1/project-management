...
export async function GET() {
  ...
  const lead = await UserModel.findOne({ email: session.user.email });
  if (!lead) return corsResponse({ message: "Lead not found" }, 404);
...
export async function POST(req: Request) {
  ...
  if (!projectId || !requestId || !["approve", "reject"].includes(action))
    return corsResponse({ message: "Invalid data" }, 400);
  ...
  if (!request) return corsResponse({ message: "Request not found" }, 404);

  if (action === "approve" && !project.members.includes(request.user)) {
    project.members.push(request.user);
  }
