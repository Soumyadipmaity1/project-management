...
export async function PATCH(req: Request) {
  try {
    ...
    if (!reqId) return corsResponse({ error: "Request id not provided in URL" }, 400);
    ...
    if (user.role === "Lead") {
      ...
      if (!reqDomains.includes(user.domain)) {
        return corsResponse({ error: "You can only manage requests in your domain" }, 403);
      }
    }
    ...
    if (leadId || action === "approve") {
      if (!allowedToApprove) return corsResponse({ error: "Forbidden" }, 403);

      if (!leadId || typeof leadId !== "string" || leadId.trim() === "") {
        return corsResponse({ error: "teamlead id is required for approval" }, 400);
      }

      const leadUser = await UserModel.findById(String(leadId).trim());
      if (!leadUser) return corsResponse({ error: "Assigned project lead user not found" }, 400);

      let coLeadUser = null;
      if (coLeadId && typeof coLeadId === "string" && coLeadId.trim() !== "") {
        coLeadUser = await UserModel.findById(String(coLeadId).trim());
        if (!coLeadUser) return corsResponse({ error: "Assigned co-lead user not found" }, 400);
      }
      ...
      try {
        ...
        leadUser.projects = leadUser.projects || [];
        const alreadyAdded = leadUser.projects.some((p: any) => p.projectId?.toString() === newProject._id.toString());
        if (!alreadyAdded) {
      ...
      if (coLeadUser) {
        try {
          ...
          coLeadUser.projects = coLeadUser.projects || [];
          const alreadyAdded = coLeadUser.projects.some((p: any) => p.projectId?.toString() === newProject._id.toString());
          if (!alreadyAdded) {
      ...
      return corsResponse(
        { message: "Request approved successfully, project created", project: newProject, request: populatedRequest },
        200
      );
    }

    if (action === "reject" || (!leadId && !action)) {
      if (!allowedToReject) return corsResponse({ error: "Forbidden" }, 403);

      const updatedRequest = await RequestModel.findByIdAndUpdate(reqId, { status: "Rejected" }, { new: true });
      return corsResponse({ message: "Request rejected successfully", updatedRequest }, 200);
    }
