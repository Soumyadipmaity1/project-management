  ...
  try {
    ...
    const formatted = projects.map((p: any) => ({
      ...
      projectlead: p.projectlead
        ? { _id: String(p.projectlead._id), name: p.projectlead.name, email: p.projectlead.email, role: p.projectlead.role }
        : null,
      colead: p.colead
        ? { _id: String(p.colead._id), name: p.colead.name, email: p.colead.email, role: p.colead.role }
        : null,
    ...
    console.error("Error fetching projects:", error);
    return corsResponse({ message: "Error fetching projects", error }, 500);
  }
}
...
export async function POST(req: Request) {
  try {
    ...
    if (!canProject(session.user.role, "create")) return corsResponse({ error: "Forbidden" }, 403);
    ...
    const domain = JSON.parse(domainRaw);
    if (!Array.isArray(domain) || !domain.length)
      return corsResponse({ error: "Invalid domain data" }, 400);

    if (title.length < 3)
      return corsResponse({ error: "Title must be at least 3 characters" }, 400);

    if (description.length < 10)
      return corsResponse({ error: "Description must be at least 10 characters" }, 400);

    const projectleadId = await resolveUserIdentifier(projectleadVal || session.user._id);
    if (!projectleadId)
      return corsResponse({ error: "Team lead not found" }, 400);
    ...
    if (completionDate < startDate)
      return corsResponse({ error: "Completion date cannot be before start date" }, 400);
    ...
    return corsResponse(project, 201);
  } catch (error) {
    console.error("Error creating project:", error);
    return corsResponse({ error: "Failed to create project", details: String(error) }, 500);
  }
}
