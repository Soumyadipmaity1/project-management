...
//   // Fetch all pending requests for the logged-in lead
//   useEffect(() => {
//     const fetchRequests = async () => {
//       try {
//         setLoading(true);
...
//     ...new Set(requests.map((req) => req.project?.title || "Unknown")),
//   ];

//   // Filter requests
//   const filteredRequests = requests.filter((request) => {
//     const projectMatch =
//       projectFilter === "all" || request.project?.title === projectFilter;
//     const searchMatch =
//       searchQuery.trim() === "" ||
...
//     try {
//       const res = await fetch(`/api/projects/requests/${id}`, {
//         method: "PUT",
//         headers: { "Content-Type": "application/json" },
...
//   return (
//     <div className="py-6 lg:px-4 min-h-screen">
//       <h2 className="text-4xl font-bold text-white">Pending Requests</h2>
//       <p className="text-gray-300 text-lg mb-8">
//         Approve or reject member contribution requests
//       </p>
...
//               <div
//                 key={request._id}
//                 className="bg-gray-900 border border-gray-800 rounded-lg flex flex-col px-6 py-6 shadow-2xl hover:bg-gray-800 transition-colors max-w-sm"
//               >
//                 <div className="flex justify-center mb-4">
//                   <img
//                     src={
//                       request.user.profilePic ||
//                       "https://i.pravatar.cc/100?u=" + request.user.email
//                     }
...
//                 <div className="text-gray-200 text-[14px] font-mclaren mb-2">
//                   <span className="text-gray-400">Email:</span>{" "}
//                   <span className="font-semibold text-fuchsia-400 break-all">
//                     {request.user.email}
//                   </span>
//                 </div>
...
export default function LeadRequestsPage() {
  ...
  return (
    <div className="p-6">
      ...
      {projects.length === 0 ? (
        <p className="text-gray-400">No pending requests yet.</p>
      ) : (
        projects.map((proj) => (
          <div
            key={proj._id}
            className="border border-fuchsia-500/30 rounded-lg p-4 mb-4"
          >
            ...
            {proj.requests.length === 0 ? (
              <p className="text-gray-400 text-sm mt-2">No requests.</p>
            ) : (
              <ul className="mt-2 space-y-2">
                {proj.requests.map((req: any) => (
                  <li
                    key={req._id}
                    className="bg-slate-800/60 rounded-md p-2 flex items-center justify-between"
                  >
                    <div>
                    ...
                    <div className="flex gap-2">
                      <button
                        onClick={() => handleApprove(proj._id, req._id)}
                        className="bg-green-600 px-3 py-1 rounded text-white text-sm"
                      >
                        Approve
                      </button>
                      <button
                        onClick={() => handleReject(proj._id, req._id)}
...
async function handleApprove(projectId: string, requestId: string) {
  const res = await fetch(`/api/lead/requests/${projectId}/approve`, {
    method: "PUT",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ requestId }),
  });
  if (res.ok) toast.success("Request approved");
  else toast.error("Failed to approve");
}

async function handleReject(projectId: string, requestId: string) {
  const res = await fetch(`/api/lead/requests/${projectId}/reject`, {
    method: "PUT",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ requestId }),
  });
